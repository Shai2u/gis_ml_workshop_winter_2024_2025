...
def update_output(contents, filename):
    ...
    if contents is not None:
        ...
        try:
            ...
            location = df['text'].apply(
                lambda t: extract_most_common_location(t, nlp))
            print('print loc[0]', location[0])
            print('loc lisgt', location)
            geo_located_points = pd.Series(location).apply(
                lambda p: geolocate_text(p)['point'])
            ...
            map_fig = generate_map(
                geo_located_points_lat, geo_located_points_lon, location.values.tolist(), result)
