...
United_states_synonyms = ["U.S", "America",
                          "States", "US", "U.S.", "the United States"]
...
def extract_most_common_location(text, nlp):
    ...
    locations = [
        "United States" if loc in United_states_synonyms else loc for loc in locations]

    # If locations were found, count their occurrences and find the most common one
    if locations:
        # Count the frequency of each location
        location_counts = Counter(locations)
        most_common_location = location_counts.most_common(
            1)[0][0]  # Find the most common location
        ...
        if most_common_location == "United States":
            if len(location_counts) > 1:  # Check if there's another location available
                most_common_location = location_counts.most_common(
                    2)[1][0]  # Return second most common location
            else:
...
def update_output(n_clicks, value):
    ...
    if n_clicks > 0:
        ...
        location = extract_most_common_location(value, nlp)

        if result[0] == 1:
            latlnog = geolocate_text(location)['point']
            return_text = f"This is a real news article. {location} {latlnog}"
            map_fig = generate_map(
                lat=latlnog[0], lon=latlnog[1], location=location, fake=False)
        else:
            latlnog = geolocate_text(location)['point']
            return_text = f"{result[0]} This is a fake news article. {location}"
            map_fig = generate_map(
                lat=latlnog[0], lon=latlnog[1], location=location, fake=True)
